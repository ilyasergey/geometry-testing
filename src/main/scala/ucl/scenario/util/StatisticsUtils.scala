...
trait StatisticsUtils {
  ...
  def getLastResultPath(sDirPath: String, tName: String, st: SubmissionType): Option[String] = {
    ...
    val dir = tfres.get
    val fname = getLastSolutionFileNumber(dir, resultsPrefix)._1
...
object CheckStatisticsUtils extends StatisticsUtils {
  ...
  private def computeCheckPolygonInfos(sRoot: String): Seq[Int] = {
    val map = getCheckPolygonMap(sRoot)
...
object GuardsStatisticsUtils extends StatisticsUtils {
